def val_list_shuukatsu_get():
    ...
    try:
        ...
        if(g_val.re_run_==1 and len(g_val.url_log_his)>4):
            ...
            url_ ="https://syukatsu-kaigi.jp/companies/search"
            driver_t1.get(url_)
            
            comparer = driver_t1.find_element(By.CLASS_NAME, 'p-paginate__info').text.split('件')
            comparer = comparer[0].replace(',', '')
            comparer = comparer.replace('全', '')
            
            flag = messagebox.askyesno('確認', '検索条件を選択する場合は[はい」を、全検索を行う場合は「いいえ」を押してください。')
            if flag:
                while True:
                    try:
                        result_num = driver_t1.find_element(By.CLASS_NAME, 'p-paginate__info').text.split('件')
                        result_num = result_num[0].replace(',', '')
                        result_num = result_num.replace('全', '')
                        if comparer != result_num:
                            break
                    except: 
                        pass
            else:
        ...
        while(1):#カテゴリ
            ...
            try:
                ...
                g_val.window_log["text"] = main_ulr
                print('main_url: ', main_ulr)
                g_val.data_log_write(g_val.window_log["text"])
                ...
                try:
                    while True:
                        kategory = g_val.driver.find_elements(By.CLASS_NAME,'p-search-panel__company-summary')[index].text
                        if kategory:
                            break
                    if len(kategory.split('\n')) > 1:
                        kategory = kategory.split('\n')[1]
                    kategory = kategory.replace('業界：','')
                    kategory = kategory.split('(')[0]
                    # print("カテゴリ取得", kategory)     
                except:
                    pass
            
                els =g_val.driver.find_elements(By.CLASS_NAME, "p-search-panel__block-link")
                ...
                while(1):
                    ...
                    if(len(els)>0):
                        ...
                        for index in range(len(els)):
                            try:
                                try:
                                    url = els[index].get_attribute('href')
                                except:
                                    url = ''
                                    pass
                                try:
                                    while True:
                                        company = g_val.driver.find_elements(By.CLASS_NAME, 'p-search-panel__heading')[index].text
                                        if company:
                                            break
                                except:
                                    company = ''
                ...
                g_val.driver = webdriver.Chrome(options=g_val.option,service=chrome_service)
                g_val.driver.get(main_ulr)
                els =g_val.driver.find_elements(By.XPATH,'//li[@class="next"]')
                st = time.time()
                while(1):
                    now = time.time() - st
                    if(now>20):break
                    if(len(els)>0):
                        ...
                        # els = g_val.driver.find_elements(By.XPATH,'//li[@class="pager_item pager_next"]/a')
                            reserch_end=0
                            reserch_url_log_ = eln.find_element(By.XPATH,'./a').get_attribute('href')
                            break
                        break
                val_get_shuukatsu(g_val.campany_list)#######rest
...
def val_get_shuukatsu(campany_list_in):
    import traceback
    try:
        from urllib.parse import urlparse
        import re
        import g_val
        import g_val_shuukatsu
        import g_fnc_google
        import random
        import csv
        import time
            ...
            try:
                ...
                if(g_val.Op_ok==0):
                    if("Windows"==g_val.OS_VERSION):
                        ...
                        #     path_o_csv_cat_list.append(g_val.path1+"\\..\\out\\result_shuukatsu"+str(categ)+".csv")
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_shuukatsu"+str(campany_list_n[2])+".csv"
                    else:
                        #path_o_csv_cat  = g_val.path1+"/../out/result_"+str(campany_list_n[2])+".csv"
                        # for categ in g_val.category:
                        #     path_o_csv_cat_list.append(g_val.path1+"/../out/result_shuukatsu"+str(categ)+".csv")
                    ...
                    if("Windows"==g_val.OS_VERSION):
                        # for categ in g_val.category:
                        #     path_o_csv_cat_list.append(g_val.path1+"\\..\\out\\result_shuukatsu_"+str(categ)+"_OP.csv")
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_shuukatsu_"+str(campany_list_n[2])+"_OP.csv"
                    else:
                        # for categ in g_val.category:
                        #     path_o_csv_cat_list.append(g_val.path1+"/../out/result_shuukatsu_"+str(categ)+"_OP.csv")
                        path_o_csv_cat  = g_val.path1+"/../out/result_shuukatsu_"+str(campany_list_n[2])+"_OP.csv"
                ...
                if(n==0):
                    ...
                    if(g_val.Op_ok==0):
                        file_o_csv.writerow(g_val_shuukatsu.shuukatsu_header_nominal)
                    else:
                        file_o_csv.writerow(g_val_shuukatsu.shuukatsu_header_op)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            shuukatsu_HP_url_ = soup.find(lambda tag: tag.name == "th" and "URL" in tag.text).find_next().text
                        except:
                            pass

                        try:
                            shuukatsu_office = soup.find(lambda tag: tag.name == "th" and "本社所在地" in tag.text).find_next().text
                            print("本社所在地：", shuukatsu_office)
                            if (re.search(" ", shuukatsu_office)):
                                txt = shuukatsu_office.split(" ")
                                for a in txt:
                            ...
                            if (re.search("　", shuukatsu_office)):
                                ...
                                for a in txt:
                                    if (re.search(r"[都道府県]", a)):
                                        shuukatsu_office = a
                                        break
                            shuukatsu_locate = shuukatsu_office
                            print("rikunabi_住所:",shuukatsu_locate)
                            #都道府県と市区町村を抽出
                            prefecture, location = extract_japanese_location(shuukatsu_locate)
                        ...
                        try:
                            shuukatsu_established = soup.find(lambda tag: tag.name == "th" and "設立日" in tag.text).find_next().text
                            print("設立：", shuukatsu_established)
                        except:
                            pass

                        try:
                            shuukatsu_president = soup.find(lambda tag: tag.name == "th" and "代表者" in tag.text).find_next().text
                            print("代表者：", shuukatsu_president)
                        except:
                            pass
                        
                        try:
                            shuukatsu_employees = soup.find(lambda tag: tag.name == "th" and "従業員数" in tag.text).find_next().text
                            print("従業員数：", shuukatsu_employees)
                        except:
                            pass

                        try:
                            shuukatsu_capital = soup.find(lambda tag: tag.name == "th" and "資本金" in tag.text).find_next().text
                            print("資本金：", shuukatsu_capital)
                        except:
                            pass

                        try:
                            shuukatsu_sales = soup.find(lambda tag: tag.name == "th" and "売上高" in tag.text).find_next().text
                            print("売上高：", shuukatsu_sales)
                        except:
                            pass

                        try:
                            shuukatsu_summary = soup.find(lambda tag: tag.name == "th" and "電話番号" in tag.text).find_next().text
                            print("電話番号：", shuukatsu_summary)
                        except:
                            pass
                    ...
                    #カテゴリ
                    #####################################################################
                    url_g ="https://www.google.co.jp/maps/search/"+company_+" "+shuukatsu_locate + "?gl=jp&h1=jp&gws_rd=cr"
                    while(1):
                        handle_array = g_val.driver.window_handles
                        if(len(handle_array)==1):break
                        else:
                    ...
                    main_ulr ="-"
                #　　　　google 検索開始
                print("shuukatsu_HP_url:", shuukatsu_HP_url_)
                out_google_list = g_fnc_google.google_detail_reserch(url_g ,campany_list_n,shuukatsu_HP_url_)
                if(g_val.stop_==1):return 0
                ...
                if(out_ok==1):
                    ...
                    if shuukatsu_locate != '-':
                        list_out_all_a.append(shuukatsu_locate)
                        list_out_all_db.append(shuukatsu_locate)
                    else:
                    ...
                    list_out_all_a.append(shuukatsu_established)
                    list_out_all_db.append(shuukatsu_established)
                    list_out_all_a.append(shuukatsu_president)
                    list_out_all_db.append(shuukatsu_president)
                    list_out_all_a.append(shuukatsu_employees)
                    list_out_all_db.append(shuukatsu_employees)
                    list_out_all_a.append(shuukatsu_capital)
                    list_out_all_db.append(shuukatsu_capital)
                    list_out_all_a.append(shuukatsu_sales)
                    list_out_all_db.append(shuukatsu_sales)
                    # list_out_all_a.append(shuukatsu_summary)
                    # list_out_all_db.append(shuukatsu_summary)
                    list_out_all_a.append(shuukatsu_locate)
                    list_out_all_db.append(shuukatsu_locate)
                    ...
                    if(g_val.csv_free_ok==1): 
                        ...
                        #     if("Windows"==g_val.OS_VERSION):
                        #         path_o_csv_cat  = g_val.path1+"\\..\\out\\result_shuukatsu_"+str(campany_list_n[2])+"_OP.csv"
                        #         #path_o_csv_cat  = g_val.path1+"\\..\\out\\result_shuukatsu_OP.csv"
                        #     else:
                        #         path_o_csv_cat  = g_val.path1+"/../out/result_shuukatsu_"+str(campany_list_n[2])+"_OP.csv"
