def val_list_en_get():
    ...
    try:
        ...
        if(g_val.re_run_==1 and len(g_val.url_log_his)>4):
            ...
            url_ ="https://employment.en-japan.com/wish/"
            driver_t1.get(url_)
            ...
            while(1):
                ...
                els = driver_t1.find_elements(By.ID, 'jobSearchListNum')
                if(len(els)>0):
                    for eln in els:
                        g_val.reserch_url_log = driver_t1.current_url
                        search_num = driver_t1.find_elements(By.ID, "searchCount")
                        if len(search_num):
        ...
        #sleep(100)
        while(1):#カテゴリ
            print('reserch_url_log: ', g_val.reserch_url_log)
            ...
            try:
                ...
                #         try:
                #             els = g_val.driver.find_elements(By.XPATH, "//tr[@class='job']")
                #             if len(els):
                #                 kategory_els = els[0].find_elements(By.XPATH, "//label[@class='inputSelect']")
                #                 # print(len(kategory_els))
                #                 if len(kategory_els):
                #                     for eln in kategory_els:
                #                         kategory_ = eln.text
                #                         if kategory_ == "特に指定しない":
                #                             kategory_ = ""
                #                         # print(eln.text)
                #                         break
                #         except:
                ...
                company_list = g_val.driver.find_elements(By.XPATH, "//*[@class='jobSearchListLeftArea']")
                if len(company_list):
                    # els = company_list[0].find_elements(By.XPATH, ".//*[@class='list']")
                    els = company_list[0].find_elements(By.XPATH, ".//*[contains(concat(' ', normalize-space(@class), ' '), ' list ') and (contains(concat(' ', normalize-space(@class), ' '), ' engage ') or not(contains(@class, ' ')))]")
                    ...
                    while(1):
                        ...
                        if(len(els)>0):
                            ...
                            for eln in els:
                                try:
                                    company = eln.find_element(By.CLASS_NAME, 'companyName').text
                                    url_= eln.find_element(By.XPATH, ".//*[@class='jobNameArea']/a").get_attribute('href')
                                    titel_=company
                                    titel_ = titel_.replace("正社員", "")
                ...
                g_val.driver = webdriver.Chrome(options=g_val.option,service=chrome_service)
                g_val.driver.get(main_ulr)
                els = g_val.driver.find_elements(By.CLASS_NAME, 'next.page.next')
                # els = g_val.driver.find_elements(By.XPATH, '//a[@class="next page next"]')
                # print("nextbutton:", len(els))
                st = time.time()
                while(1):
                    now = time.time() - st
                    if(now>20):break
                ...
                val_get_en(g_val.campany_list)#######rest
                print("詳細完了")
...
def val_get_en(campany_list_in):
    import traceback
    try:
        ...
        while(1):
            ...
            for _ in range(max_retries):
                try:
                    response = requests.get(campany_list_n[1])
                    break
                except ConnectionError as e:
                    print(f"ConnectionError occurred: {e}. Retrying...")
                    time.sleep(3)
            soup = BeautifulSoup(response.content, 'html.parser')
            category = soup.find_all(class_='list')[1].text
            category = category.replace('転職 ', '')
            if len(category.split('（')):
                category = category = category.split('（')[0]
            campany_list_n[2] = category
            ...
            try:
                ...
                if(g_val.Op_ok==0):
                    if("Windows"==g_val.OS_VERSION):
                        ...
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_entenshoku "+str(campany_list_n[2])+".csv"
                    else:
                        #path_o_csv_cat  = g_val.path1+"/../out/result_"+str(campany_list_n[2])+".csv"
                        path_o_csv_cat  = g_val.path1+"/../out/result_entenshoku "+str(campany_list_n[2])+".csv"
                    ...
                    if("Windows"==g_val.OS_VERSION):
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_entenshoku_"+str(campany_list_n[2])+"_OP.csv"
                        #path_o_csv_cat  = g_val.path1+"\\..\\out\\result_entenshoku_OP.csv"
                    else:
                        path_o_csv_cat  = g_val.path1+"/../out/result_entenshoku_"+str(campany_list_n[2])+"_OP.csv"
                        #path_o_csv_cat  = g_val.path1+"/../out/result_entenshoku_OP.csv"
                n=0
                ...
                if(n==0):
                    ...
                    if(g_val.Op_ok==0):
                        file_o_csv.writerow(g_val_en.en_header_nominal)
                    else:
                        file_o_csv.writerow(g_val_en.en_header_op)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            try:
                                ...
                                sub_url = soup.find(id='recruitFrame')['src']
                                sub_res = requests.get(sub_url)
                                sub_soup = BeautifulSoup(response.content, 'html.parser')
                                en_HP_url_ = sub_soup.find(lambda tag: tag.name == "th" and "WEBサイト" in tag.text).find_next().find_next().text
                            en_HP_url_ = en_HP_url_.replace('\n', '')
                            # try:
                            #     en_HP_url_ = en_HP_url_.split("\n")[1]
                            # except:
                            #     pass
                            print("企業ホームページ:", en_HP_url_)
                        except:
                            pass

                        try:
                            try:
                                en_office = soup.find(lambda tag: tag.name == "th" and "事業所" in tag.text).find_next().get_text(separator='/', strip=True)
                            except:
                                en_office = sub_soup.find(lambda tag: tag.name == "th" and "所在地" in tag.text).find_next().text
                            en_office = en_office.replace('本社', '')
                            en_office = re.sub(r'\【.*?\】', '', en_office)
                            en_office = re.sub(r'\<.*?\>', '', en_office)
                            en_office = en_office.replace('■', '')
                            try:
                                en_office = en_office.split("\n")[1]
                            except:
                                pass
                            print("事業所：", en_office)
                            if (re.search("/", en_office)):
                                txt = en_office.split("/")
                                for a in txt:
                            ...
                            if (re.search("／", en_office)):
                                ...
                                for a in txt:
                                    if (re.search(r"[都道府県]", a)):
                                        ...
                                        break
                            en_locate = en_office
                            print("en_住所:",en_locate)
                            #都道府県と市区町村を抽出
                            prefecture, location = extract_japanese_location(en_locate)
                            campany_list_n[3] = prefecture + "  " + location
                            print("en_住所:市区町村::",campany_list_n[3])
                        except:
                            print("エラー住所")
                            traceback.print_exc()
                            en_locate ="-"
                        ...
                        try:
                            try:
                                en_established = soup.find(lambda tag: tag.name == "th" and "設立" in tag.text).find_next().text
                                # try:
                                #     en_established = en_established.split("\n")[1]
                                # except:
                                #     pass
                            except:
                                en_established = soup.find(lambda tag: tag.name == "th" and "創業" in tag.text).find_next().text
                            print("設立：", en_established)
                        except:
                            pass

                        try:
                            en_president = soup.find(lambda tag: tag.name == "th" and "代表者" in tag.text).find_next().text
                            # try:
                            #     en_president = en_president.split("\n")[1]
                            # except:
                            #     pass
                            print("代表者：", en_president)
                        except:
                            pass
                        
                        try:
                            en_employees = soup.find(lambda tag: tag.name == "th" and "従業員数" in tag.text).find_next().text
                            # try:
                            #     en_employees = en_employees.split("\n")[1]
                            # except:
                            #     pass
                            print("従業員数：", en_employees)
                        except:
                            pass

                        try:
                            en_capital = soup.find(lambda tag: tag.name == "th" and "資本金" in tag.text).find_next().text
                            # try:
                            #     en_capital = en_capital.split("\n")[1]
                            # except:
                            #     pass
                            print("資本金：", en_capital)
                        except:
                            pass

                        try:
                            en_sales = soup.find(lambda tag: tag.name == "th" and "売上高" in tag.text).find_next().text
                            # try:
                            #     en_sales = en_sales.split("\n")[1]
                            # except:
                            #     pass
                            print("売上高：", en_sales)
                        except:
                        ...
                        try:
                            en_summary = soup.find(lambda tag: tag.name == "th" and "事業内容" in tag.text).find_next().text
                            print("事業内容：", en_summary)
                        except:
                            en_summary = sub_soup.find(lambda tag: tag.name == "th" and "事業内容" in tag.text).find_next().text
                            print("事業内容：", en_summary)
                            # try:
                            #     en_summary = en_summary.split("\n")[1]
                            # except:
                            #     pass
                    ...
                    #カテゴリ
                    #####################################################################
                    url_g ="https://www.google.co.jp/maps/search/"+company_+" "+en_locate + "?gl=jp&h1=jp&gws_rd=cr"
                    while(1):
                        handle_array = g_val.driver.window_handles
                        if(len(handle_array)==1):break
                        else:
                    ...
                    main_ulr ="-"
                #　　　　google 検索開始
                print("en_HP_url:", en_HP_url_)
                out_google_list = g_fnc_google.google_detail_reserch(url_g ,campany_list_n,en_HP_url_)
                if(g_val.stop_==1):return 0
                ...
                if(out_ok==1):
                    ...
                    if en_locate != '-':
                        list_out_all_a.append(en_locate)
                        list_out_all_db.append(en_locate)
                    else:
                    ...
                    list_out_all_a.append(en_established)
                    list_out_all_db.append(en_established)
                    list_out_all_a.append(en_president)
                    list_out_all_db.append(en_president)
                    list_out_all_a.append(en_employees)
                    list_out_all_db.append(en_employees)
                    list_out_all_a.append(en_capital)
                    list_out_all_db.append(en_capital)
                    list_out_all_a.append(en_sales)
                    list_out_all_db.append(en_sales)
                    list_out_all_a.append(en_summary)
                    list_out_all_db.append(en_summary)
                    list_out_all_a.append(en_locate)
                    list_out_all_db.append(en_locate)
