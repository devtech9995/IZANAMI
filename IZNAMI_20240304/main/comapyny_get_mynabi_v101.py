def val_list_mynabi_get():
    ...
    try:
        ...
        if(g_val.re_run_==1 and len(g_val.url_log_his)>4):
            ...
            url_ ="https://tenshoku.mynavi.jp/search"
            driver_t1.get(url_)
            ...
            while(1):
                ...
                els = driver_t1.find_elements(By.XPATH, '//span[contains(text(),"を検索する")]')
                if(len(els)>0):
                    for eln in els:
                ...
                els = driver_t1.find_elements(By.XPATH, "//button[@class='btnSearch']")
                if(len(els)>0):
                    for eln in els:
                        g_val.reserch_url_log = driver_t1.current_url
                        break
        ...
        while(1):#カテゴリ
            ...
            try:
                ...
                sleep(0.5)
                try:
                    if len(g_val.driver.find_elements(By.CLASS_NAME, "js__areaCheckbox--selected")):
                        search_num = g_val.driver.find_elements(By.XPATH, "//p[@class='result__num']")[0].text
                    else:
                        search_num = g_val.driver.find_elements(By.CLASS_NAME, "total_num")[0].text
                except:
                ...
                try:
                    ...
                    while(1):
                        ...
                        try:
                            els = g_val.driver.find_elements(By.XPATH, "//span[@class='js__jobCheckbox--selected']")
                            if len(els):
                                kategory_els = els[0].find_elements(By.XPATH, "*")
                ...
                # els = g_val.driver.find_elements(By.)
                if len(g_val.driver.find_elements(By.CLASS_NAME, "js__areaCheckbox--selected")):
                    els =g_val.driver.find_elements(By.CLASS_NAME, "boxAdjust")
                else:
                    els = g_val.driver.find_elements(By.XPATH, "//p[@class='txt']")
                ...
                while(1):
                    ...
                    if(len(els)>0):
                        ...
                        for eln in els:
                            try:
                                try:
                                    company = eln.find_element(By.XPATH, "preceding-sibling::h3").text.split('|')[0]
                                except:
                                    company = eln.find_element(By.XPATH, "preceding-sibling::p").text.split('|')[0]
                                url = eln.find_element(By.XPATH, "*").get_attribute('href')
                                url_= '/'.join(url.split('/')[:4])
                                titel_=eln.text
                ...
                g_val.driver = webdriver.Chrome(options=g_val.option,service=chrome_service)
                g_val.driver.get(main_ulr)
                els =g_val.driver.find_elements(By.XPATH,'//a[@class="iconFont--arrowLeft"]')
                if len(els) < 1:
                    els = g_val.driver.find_elements(By.XPATH,'//li[@class="pager_item pager_next"]/a')
                st = time.time()
                while(1):
                    now = time.time() - st
                    if(now>20):break
                ...
                val_get_mynabi(g_val.campany_list)#######rest
                print("詳細完了")
...
def val_get_mynabi(campany_list_in):
    import traceback
    try:
            ...
            try:
                ...
                if(g_val.Op_ok==0):
                    if("Windows"==g_val.OS_VERSION):
                        ...
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_mynavi"+str(campany_list_n[2])+".csv"
                    else:
                        #path_o_csv_cat  = g_val.path1+"/../out/result_"+str(campany_list_n[2])+".csv"
                        path_o_csv_cat  = g_val.path1+"/../out/result_mynavi"+str(campany_list_n[2])+".csv"
                    ...
                    if("Windows"==g_val.OS_VERSION):
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_mynavi_"+str(campany_list_n[2])+"_OP.csv"
                        #path_o_csv_cat  = g_val.path1+"\\..\\out\\result_mynavi_OP.csv"
                    else:
                        path_o_csv_cat  = g_val.path1+"/../out/result_mynavi_"+str(campany_list_n[2])+"_OP.csv"
                        #path_o_csv_cat  = g_val.path1+"/../out/result_mynavi_OP.csv"
                n=0
                ...
                if(n==0):
                    ...
                    if(g_val.Op_ok==0):
                        file_o_csv.writerow(g_val_mynabi.mynabi_header_nominal)
                    else:
                        file_o_csv.writerow(g_val_mynabi.mynabi_header_op)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            mynabi_HP_url_ = soup.find(lambda tag: tag.name == "th" and "企業ホームページ" in tag.text).find_next().text
                            try:
                                mynabi_HP_url_ = mynabi_HP_url_.split("\n")[1]
                            except:
                                pass
                            print("企業ホームページ：", mynabi_HP_url_)
                        except:
                            pass

                        try:
                            mynabi_office = soup.find(lambda tag: tag.name == "th" and "本社所在地" in tag.text).find_next().text
                            try:
                                mynabi_office = mynabi_office.split("\n")[1]
                            except:
                                pass
                            print("本社所在地：", mynabi_office)
                            if (re.search(" ", mynabi_office)):
                                txt = mynabi_office.split(" ")
                                for a in txt:
                            ...
                            if (re.search("　", mynabi_office)):
                                ...
                                for a in txt:
                                    if (re.search(r"[都道府県]", a)):
                                        mynabi_office = a
                                        break
                            mynabi_locate = mynabi_office
                            print("mynabi_住所:",mynabi_locate)
                            #都道府県と市区町村を抽出
                            prefecture, location = extract_japanese_location(mynabi_locate)
                            campany_list_n[3] = prefecture + "  " + location
                            print("mynabi_住所:市区町村::",campany_list_n[3])
                        except:
                            print("エラー住所")
                            traceback.print_exc()
                            mynabi_locate ="-"
                        ...
                        try:
                            mynabi_established = soup.find(lambda tag: tag.name == "th" and "設立" in tag.text).find_next().text
                            try:
                                mynabi_established = mynabi_established.split("\n")[1]
                            except:
                                pass
                            print("設立：", mynabi_established)
                        except:
                            pass

                        try:
                            mynabi_president = soup.find(lambda tag: tag.name == "th" and "代表者" in tag.text).find_next().text
                            try:
                                mynabi_president = mynabi_president.split("\n")[1]
                            except:
                                pass
                            print("代表者：", mynabi_president)
                        except:
                            pass
                        
                        try:
                            mynabi_employees = soup.find(lambda tag: tag.name == "th" and "従業員数" in tag.text).find_next().text
                            try:
                                mynabi_employees = mynabi_employees.split("\n")[1]
                            except:
                                pass
                            print("従業員数：", mynabi_employees)
                        except:
                            pass

                        try:
                            mynabi_capital = soup.find(lambda tag: tag.name == "th" and "資本金" in tag.text).find_next().text
                            try:
                                mynabi_capital = mynabi_capital.split("\n")[1]
                            except:
                                pass
                            print("資本金：", mynabi_capital)
                        except:
                            pass

                        try:
                            mynabi_sales = soup.find(lambda tag: tag.name == "th" and "売上高" in tag.text).find_next().text
                            try:
                                mynabi_sales = mynabi_sales.split("\n")[1]
                            except:
                                pass
                            print("売上高：", mynabi_sales)
                        except:
                            pass

                        try:
                            mynabi_summary = soup.find(lambda tag: tag.name == "th" and "事業内容" in tag.text).find_next().text
                            try:
                                mynabi_summary = mynabi_summary.split("\n")[1]
                            except:
                                pass
                            print("事業内容：", mynabi_summary)
                        except:
                            pass
                    ...
                    #カテゴリ
                    #####################################################################
                    url_g ="https://www.google.co.jp/maps/search/"+company_+" "+mynabi_locate + "?gl=jp&h1=jp&gws_rd=cr"
                    while(1):
                        handle_array = g_val.driver.window_handles
                        if(len(handle_array)==1):break
                        else:
                    ...
                    main_ulr ="-"
                #　　　　google 検索開始
                print("mynabi_HP_url:", mynabi_HP_url_)
                out_google_list = g_fnc_google.google_detail_reserch(url_g ,campany_list_n,mynabi_HP_url_)
                if(g_val.stop_==1):return 0
                ...
                if(out_ok==1):
                    ...
                    if mynabi_locate != '-':
                        list_out_all_a.append(mynabi_locate)
                        list_out_all_db.append(mynabi_locate)
                    else:    
                    ...
                    list_out_all_a.append(mynabi_established)
                    list_out_all_db.append(mynabi_established)
                    list_out_all_a.append(mynabi_president)
                    list_out_all_db.append(mynabi_president)
                    list_out_all_a.append(mynabi_employees)
                    list_out_all_db.append(mynabi_employees)
                    list_out_all_a.append(mynabi_capital)
                    list_out_all_db.append(mynabi_capital)
                    list_out_all_a.append(mynabi_sales)
                    list_out_all_db.append(mynabi_sales)
                    list_out_all_a.append(mynabi_summary)
                    list_out_all_db.append(mynabi_summary)
                    list_out_all_a.append(mynabi_locate)
                    list_out_all_db.append(mynabi_locate)
