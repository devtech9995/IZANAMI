def val_list_dud_campany_get():
    ...
    try:
        ...
        if(g_val.re_run_==1 and len(g_val.url_log_his)>4):
            ...
            url_ ="https://doda.jp/DodaFront/View/CompanySearchDetail/j_from_global_navi__True/"
            driver_t1.get(url_)
            ...
            while(1):
                ...
                els = driver_t1.find_elements_by_xpath('//p[contains(text(),"該当企業数")]')
                if(len(els)>0):
                    for eln in els:
            ...
            try:
                ...
                try:
                    ...
                    while(1):
                        ...
                        els = g_val.driver.find_elements(By.XPATH,'//div[@class="sideMenu__sectionHeader"]//h4[contains(text(),"業種")]/../../div//span[@class="checkbox__text"]')
                ...
                els =g_val.driver.find_elements(By.XPATH,'//h2[@class="title"]')
                ...
                while(1):
                    ...
                    if(len(els)>0):
                        ...
                        for eln in els:
                            try:
                                url_= eln.find_element_by_xpath('a').get_attribute('href')
                ...
                while(1):
                    now = time.time() - st
                    els =g_val.driver.find_elements(By.XPATH,'//button[@class="pager__buttonNext pager__button"]/../../a')
                    if(now>20):break
                    if(len(els)>0):
                        print(len(els))
                        for eln in els:
                            ...
                            break
                        break
                print("Next URL ",reserch_url_log_)
            ...
            try:
                ...
                if(g_val.Op_ok==0):
                    if("Windows"==g_val.OS_VERSION):
                        ...
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_duda"+str(campany_list_n[2])+".csv"
                    else:
                        #path_o_csv_cat  = g_val.path1+"/../out/result_"+str(campany_list_n[2])+".csv"
                        path_o_csv_cat  = g_val.path1+"/../out/result_duda"+str(campany_list_n[2])+".csv"
                    ...
                    if("Windows"==g_val.OS_VERSION):
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_duda_"+str(campany_list_n[2])+"_OP.csv"
                ...
                try:
                    ...
                    try:
                        ...
                        while(1):
                            now = time.time() - st
                            if(now>3):break
                            els = g_val.driver.find_elements_by_xpath('//*[contains(text(),"事業内容")]/..//td')
                            if(len(els)>0):
                                for eln in els:
                                    duda_summary=eln.text
                                break
                            els1 = g_val.driver.find_elements_by_xpath('//*[contains(text(),"事業内容")]/..//dd')
                            if(len(els1)>0):
                                for eln in els1:
                                    duda_summary=eln.text
                                break
