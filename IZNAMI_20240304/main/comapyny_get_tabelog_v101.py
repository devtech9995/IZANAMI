def val_list_tabelog_get():
    ...
    try:
        ...
        if(g_val.re_run_==1 and len(g_val.url_log_his)>4):
            ...
            url_ ="https://tabelog.com/rstLst/search/"
            driver_t1.get(url_)
            flag = messagebox.askyesno('確認', '最大1200件のリストを収集します。よろしいですか？\n※食べログの仕様上一度に1200件までしか表示・収集できません。')
            
            # messagebox.askyesno('確認', '食べログの仕様上一度に1200件までしか表示・収集できません。')
            if flag:
                while True:
                    try:
                        comparer = driver_t1.find_elements(By.CLASS_NAME, 'c-page-count__num')[-1].text
                        old_url = driver_t1.current_url
                        if int(comparer)<=1200:
                            result_num = comparer
                            print('検索件数：', result_num)
                            flag1 = messagebox.askyesno('確認', '収集を開始します。よろしいですか？')
                            if flag1:
                                break
                            else:
                                while driver_t1.current_url == old_url:
                                    continue
                    except:
                        pass
            else:
        ...
        while(1):#カテゴリ
            ...
            try:
                ...
                kategory=""
                els = g_val.driver.find_elements(By.CLASS_NAME, 'list-condition__condition-target')
                if len(els):
                    kategory = els[0].text
                ...
                els =g_val.driver.find_elements(By.CLASS_NAME, 'list-rst__rst-name-target')
                ...
                # print(g_val.campany_list)
                # print("*******************************************")
                val_get_tabelog(g_val.campany_list)#######rest
                ...
                g_val.driver = webdriver.Chrome(options=g_val.option,service=chrome_service)
                g_val.driver.get(main_ulr)
                els =g_val.driver.find_elements(By.CLASS_NAME,'c-pagination__arrow--next')
                st = time.time()
                while(1):
                    now = time.time() - st
                    if(now>20):break
...
def val_get_tabelog(campany_list_in):
    import traceback
    try:
        from urllib.parse import urlparse
        import re
        import g_val
        import g_val_tabelog
        import g_fnc_google
        import random
        import csv
        import time
        ...
        while(1):
            ...
            # try:
            #     campany_list_n[2] = soup.find(lambda tag: tag.name == "th" and "ジャンル" in tag.text).find_next().text.split('、')[0].replace('\n', '')
            # except:
            #     pass
            ...
            try:
                ...
                if(g_val.Op_ok==0):
                    if("Windows"==g_val.OS_VERSION):
                        ...
                        #     path_o_csv_cat_list.append(g_val.path1+"\\..\\out\\result_tabelog"+str(categ)+".csv")
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_tabelog"+str(campany_list_n[2])+".csv"
                    else:
                        #path_o_csv_cat  = g_val.path1+"/../out/result_"+str(campany_list_n[2])+".csv"
                        # for categ in g_val.category:
                        #     path_o_csv_cat_list.append(g_val.path1+"/../out/result_tabelog"+str(categ)+".csv")
                    ...
                    if("Windows"==g_val.OS_VERSION):
                        # for categ in g_val.category:
                        #     path_o_csv_cat_list.append(g_val.path1+"\\..\\out\\result_tabelog_"+str(categ)+"_OP.csv")
                        path_o_csv_cat  = g_val.path1+"\\..\\out\\result_tabelog_"+str(campany_list_n[2])+"_OP.csv"
                    else:
                        # for categ in g_val.category:
                        #     path_o_csv_cat_list.append(g_val.path1+"/../out/result_tabelog_"+str(categ)+"_OP.csv")
                        path_o_csv_cat  = g_val.path1+"/../out/result_tabelog_"+str(campany_list_n[2])+"_OP.csv"
                ...
                if(n==0):
                    ...
                    if(g_val.Op_ok==0):
                        file_o_csv.writerow(g_val_tabelog.tabelog_header_nominal)
                    else:
                        file_o_csv.writerow(g_val_tabelog.tabelog_header_op)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            tabelog_HP_url_ = soup.find(lambda tag: tag.name == "th" and "ホームページ" in tag.text).find_next().text
                        except:
                            pass

                        try:
                            tabelog_office = soup.find(lambda tag: tag.name == "th" and "住所" in tag.text).find_next().text
                            print("住所", tabelog_office)
                            if (re.search(" ", tabelog_office)):
                                txt = tabelog_office.split(" ")
                                for a in txt:
                            ...
                            if (re.search("　", tabelog_office)):
                                ...
                                for a in txt:
                                    if (re.search(r"[都道府県]", a)):
                                        tabelog_office = a
                                        break
                            tabelog_locate = tabelog_office
                            print("rikunabi_住所:",tabelog_locate)
                            #都道府県と市区町村を抽出
                            prefecture, location = extract_japanese_location(tabelog_locate)
                        ...
                        try:
                            tabelog_open = soup.find(lambda tag: tag.name == "th" and "オープン日" in tag.text).find_next().text
                            print("設立：", tabelog_open)
                        except:
                            pass

                        try:
                            tabelog_contributor = soup.find(lambda tag: tag.name == "th" and "初投稿者" in tag.text).find_next().text
                            print("初投稿者：", tabelog_contributor)
                        except:
                            pass
                        
                        try:
                            tabelog_phone = soup.find(lambda tag: tag.name == "th" and "電話番号" in tag.text).find_next().text
                            print("電話番号：", tabelog_phone)
                        except:
                            pass
                    ...
                    #カテゴリ
                    #####################################################################
                    url_g ="https://www.google.co.jp/maps/search/"+company_+" "+tabelog_locate + "?gl=jp&h1=jp&gws_rd=cr"
                    while(1):
                        handle_array = g_val.driver.window_handles
                        if(len(handle_array)==1):break
                        else:
                    ...
                    main_ulr ="-"
                #　　　　google 検索開始
                print("tabelog_HP_url:", tabelog_HP_url_)
                out_google_list = g_fnc_google.google_detail_reserch(url_g ,campany_list_n,tabelog_HP_url_)
                if(g_val.stop_==1):return 0
                ...
                if(out_ok==1):
                    ...
                    list_out_all_a.append(tabelog_open)
                    list_out_all_db.append(tabelog_open)
                    list_out_all_a.append(tabelog_phone)
                    list_out_all_db.append(tabelog_phone)
                    list_out_all_a.append(tabelog_contributor)
                    list_out_all_db.append(tabelog_contributor)
                    # list_out_all_a.append(tabelog_capital)
                    # list_out_all_db.append(tabelog_capital)
                    # list_out_all_a.append(tabelog_sales)
                    # list_out_all_db.append(tabelog_sales)
                    # list_out_all_a.append(tabelog_summary)
                    # list_out_all_db.append(tabelog_summary)
                    list_out_all_a.append(tabelog_locate)
                    list_out_all_db.append(tabelog_locate)
                    ...
                    'open':list_out_all_db[21],
                    'phone':list_out_all_db[22],
                    'contributor':list_out_all_db[23],
                    ...
                    if(g_val.csv_free_ok==1): 
                        ...
                        #     if("Windows"==g_val.OS_VERSION):
                        #         path_o_csv_cat  = g_val.path1+"\\..\\out\\result_tabelog_"+str(campany_list_n[2])+"_OP.csv"
                        #         #path_o_csv_cat  = g_val.path1+"\\..\\out\\result_tabelog_OP.csv"
                        #     else:
                        #         path_o_csv_cat  = g_val.path1+"/../out/result_tabelog_"+str(campany_list_n[2])+"_OP.csv"
